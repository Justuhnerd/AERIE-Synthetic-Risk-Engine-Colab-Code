...
def call_gemini_api(prompt, api_key):
    # Try gemini-2.0-flash first, fall back to gemini-1.5-flash-8b (higher free quota)
    models = [
        "gemini-2.0-flash",
        "gemini-1.5-flash-8b",
        "gemini-1.5-flash",
    ...
    for model_name in models:
        ...
        try:
            ...
            if r.status_code == 429:
                last_error = f"Rate limit hit on {model_name}, trying next model…"
            ...
            last_error = str(e)
            continue
    raise Exception(f"All Gemini models rate-limited or unavailable. Try again in a few minutes.\nLast error: {last_error}")
